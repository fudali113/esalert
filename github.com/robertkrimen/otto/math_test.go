...
func TestMath_log(t *testing.T) {
	tt(t, func() {
		...
		test(`Math.mylog(NaN)`, _NaN)
		test(`Math.mylog(-1)`, _NaN)
		test(`Math.mylog(+0)`, -_Infinity)
		test(`Math.mylog(-0)`, -_Infinity)
		test(`1/Math.mylog(1)`, _Infinity)
		test(`Math.mylog(Infinity)`, _Infinity)

		test(`Math.mylog(0.5)`, -0.6931471805599453)
	})
}
